...
| `pose_mode` | `auto` (default) fully automatic pose search / `center-rotation` human-seeded / `corners` (not yet implemented upstream) |
...
| `pose_json` | optional alternative to the three fields above: `{"center_um":[x,y],"rotation_deg":r,"scale":s}` |
...
The subject is resolved by glob via `xenium_autocoreg.config.resolve_subject` (no IDs
hard-coded -- attach per subject, or trigger via a pipeline monitor):
...
- This mirrors the [`capsule-2p-3DFISH-autocoreg`](https://github.com/AllenNeuralDynamics/capsule-2p-3DFISH-autocoreg)
  ...
  [`2p2fish`](https://github.com/jkim0731/2p2fish).
...
- `pose_mode=corners` is not implemented upstream and will fail the run.
